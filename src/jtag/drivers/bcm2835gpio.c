...
{
	return (bool)((BCM2835_GPIO_REG_READ((13 + (gpio_pin_num / 32))) >> (gpio_pin_num % 32)) & 1);
}
...
{
	bool value = bcm2835_get_level(adapter_gpio_config[ADAPTER_GPIO_IDX_TDO].gpio_num);
...
static void bcm2835_swdio_drive(bool is_output)
{
	if (is_output) {
		...
		bcm2835_set_output(adapter_gpio_config[ADAPTER_GPIO_IDX_SWDIO].gpio_num);
	} else {
...
{
	bool value = bcm2835_get_level(adapter_gpio_config[ADAPTER_GPIO_IDX_SWDIO].gpio_num);
